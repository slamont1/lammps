...
FixBondDynamic::FixBondDynamic(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  distsq(nullptr), probabilities(nullptr), influenced(nullptr), list(nullptr),
...
void FixBondDynamic::init() //POSSIBLY ADD NEWTON_BOND FLAG!!!!!
{
  ...
  if (force->pair == nullptr || cutsq > force->pair->cutsq[iatomtype][jatomtype])
    error->all(FLERR,"Fix bond/dynamic cutoff is longer than pairwise cutoff");
...
void FixBondDynamic::setup(int /*vflag*/)
{
  ...
  memory->create(influenced,nmax,"bond/dynamic:influenced");
}
...
void FixBondDynamic::rebuild_special_one(int m)
{
...
int FixBondDynamic::dedup(int nstart, int nstop, tagint *copy)
{
  int i;
