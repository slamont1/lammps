...
BondNonlinearNew::BondNonlinearNew(LAMMPS *lmp) : Bond(lmp)
{
...
BondNonlinearNew::~BondNonlinearNew()
{
  if (allocated) {
...
void BondNonlinearNew::compute(int eflag, int vflag)
{
  int i1,i2,n,type;
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    double r2tol = 0.98;
    double drtol = sqrt(r2tol*lamdasq);
    if (drsq/lamdasq > r2tol) {
        dr = drtol;
        drsq = dr*dr;
        lamdasq = lamda[type]*lamda[type];
        denom = lamdasq - drsq;
...
void BondNonlinearNew::allocate()
{
  allocated = 1;
...
void BondNonlinearNew::coeff(int narg, char **arg)
{
...
double BondNonlinearNew::equilibrium_distance(int i)
{
  return r0[i];
}
...
void BondNonlinearNew::write_restart(FILE *fp)
{
...
void BondNonlinearNew::read_restart(FILE *fp)
{
  allocate();
...
void BondNonlinearNew::write_data(FILE *fp)
{
...
double BondNonlinearNew::single(int type, double rsq, int /*i*/, int /*j*/,
                             double &fforce)
{
  double r = sqrt(rsq);
  double dr = r - r0[type];
...
void BondNonlinearNew::born_matrix(int type, double rsq, int /*i*/, int /*j*/, double &du, double &du2)
{
  double r = sqrt(rsq);
  double dr = r - r0[type];
  double drsq = dr * dr;
...
void *BondNonlinearNew::extract(const char *str, int &dim)
{
  dim = 1;
